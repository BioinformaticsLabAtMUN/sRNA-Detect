...
#    Refactored by Shuaishuai Li (shuaishuail@mun.ca),
#    Memorial University of Newfoundland. Copyright 2021.
#
...
#   Reads a list of sam/bam files (multiple sequencing runs for same organism)
...
def readCommandLine():
    ...
    parser.add_argument('-minLen', action='store', default=20, type=int,
                        help='minimum length to consider for small transcripts')
    parser.add_argument('-maxLen', action='store', default=210, type=int,
                        help='maximum length to consider for small transcripts')
    parser.add_argument('-minH', action='store', default=10, type=int,
                        help='minimum number of reads across all samples required to detect small transcripts')
    parser.add_argument('-maxPDV', action='store', default=0.5, type=float,
                        help='maximum allowed drop in coverage wrt current coverage')
    parser.add_argument('-maxPN', action='store', default=-0.01, type=float,
                        help='maximum allowed negative change (percentage) in mean coverage')
    parser.add_argument('-maxPP', action='store', default=0.03, type=float,
                        help='maximum allowed positive change (percentage) in mean coverage')
    parser.add_argument('-idPrefix', action='store', default="sRNA",
                        help='Prefix to be used as identifier for sRNAs in gtf file')
...
def read_SAM(filename):
    print('reading and parsing file', filename, 'in process ID:', os.getpid())
    # read and parse the given SAM/BAM file
    alignment_file = HTSeq.BAM_Reader(filename)
    ivList = []
    for alngt in alignment_file:
        if alngt.aligned:
            # collecting the genomic interval that the read was aligned to
            ivList.append(alngt.iv)
    # return the genomic interval list
    return ivList


def detect_sRNA(arf, cvg):
    # Variables
    start = 0
    end = 0
    meanHeight = 0
    ...
    for s in sRNAs:
        fields = s[0].chrom + "\tsRNADetect\tsRNA\t" + str(s[0].start + 1) + "\t" + str(s[0].end + 1) + \
            ...
            counter) + "\"; unique_id \"" + args.idPrefix + '%05d' % counter + "\"\n"
        myfile.write(fields)
        counter += 1
    myfile.close()
...
if __name__ == '__main__':
    ...
    with Pool(processes=len(samFiles)) as pool:
        multiple_results = [pool.apply_async(read_SAM, (name,)) for name in samFiles]
        for res in multiple_results:
            for iv in res.get():
                cvg[iv] += 1

    ## start detecting sRNAs
    detect_sRNA(args, cvg)